...
experimental and is capability-supported only from Podman 5.7.0 through 6.1.0; the generator fixture proves
pre-5.7 exclusion plus command construction only. `ImageVolume` is a source-preserving singleton,
explicitly unsupported through 6.0.2 and native only at 6.1.0; its dedicated fixture proves final
authored-value selection and one separate `--image-volume tmpfs` argument. Network/Image completion fixtures record
...
`--user`, `--userns`, `--group-add`, `--workdir`, and `--read-only` output across all 22 recorded
...
Image document. The 22-release dry-run fixtures record source pulls and target-only ImageTag
...
5.4.0 through 6.1.0 observe one final effective `File` command argument, but that does not alter
...
native from 5.7.0 through 6.1.0, where a separate fixture proves `key=value` and empty-value `key=` forms; it remains
opaque without assignment parsing or environment/secret resolution, and makes no bare/null claim.
Build `Secret` is native only from 5.4.0 through 6.1.0 and stays opaque: it does not parse commas,
...
Build `Pull` is native only from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated
...
Build `TLSVerify` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated
...
Build `ForceRM` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated
...
Build `GroupAdd` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated
...
Build `DNS` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated fixture
...
Build `DNSSearch` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated
...
Build `AuthFile` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated
...
Build `IgnoreFile` is unsupported from 5.4.0 through 5.6.2, native from 5.7.0 through 6.1.0,
...
Build `PodmanArgs` has finite native evidence from 5.4.0 through 6.1.0. Its all-22 fixtures prove only

Build `Annotation` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its isolated
...
Build `Environment` is native from 5.4.0 through 6.1.0 and unknown outside that range. Its
...
Build `ContainersConfModule` is native from 5.4.0 through 6.1.0 and unknown outside that range.
...
image from every recorded Podman 5.4.0 through 6.1.0 release; support is unknown outside that
...
opaque physical lines. A separate render-owned `EnvironmentAssignment` wrapper safely emits one
literal assignment with an ASCII name and a whole double-quoted value, escaping only quotes and
backslashes. `EnvironmentAssignments` combines only prevalidated wrappers into one non-empty
physical entry, retaining order and separating their whole quoted spellings with one ASCII space.
...
one group through Podman 5.4.0–6.1.0; it records literal space on 5.4.x and `\x20` later. Percent
...
`--pids-limit 127` and `--pids-limit -1` output across Podman 5.4.0 through 6.1.0. They do not
...
patch releases from 5.5.0 through 6.1.0 emits exactly one final `--memory 16777216b`. This does not
...
`ExecReload` form, 5.6.0–6.1.0 generated-name form, final blank omission and malformed-command
tokenization, and
...
Network `Label` is also opaque, repeatable, and resettable from 5.4.0 through 6.1.0. Its source
...
Volume `Label` is likewise opaque, repeatable, and resettable from 5.4.0 through 6.1.0. Its source
entries retain empties, duplicates, bare values, embedded equals signs, quotes, specifiers,
...
Volume `GlobalArgs` is opaque, repeatable physical-line text, native from 5.4.0 through 6.1.0 and
...
Volume `PodmanArgs` is opaque, repeatable physical-line text, native from 5.4.0 through 6.1.0 and
`unknown` outside that range. Its full matrix records only target empty reset,
...
Volume `User` is opaque singleton physical-line text, native from 5.4.0 through 6.1.0 and unknown
outside that range. The all-22 fixture observes only unambiguous `User=123` becoming `o=uid=123`
before the volume name; no UID/name parsing, ownership, mount, filesystem, runtime, Compose, or
conversion behavior is claimed.

Volume `Group` is opaque singleton physical-line text, native from 5.4.0 through 6.1.0 and unknown
outside that range. The all-22 fixture observes only unambiguous `Group=456` becoming `o=gid=456`
...
from 6.0.0 through 6.1.0. The four-version fixture observes exactly one `--gid 5678` before the terminal
...
| --------------------------------------------------------------- | ----------- | ----------------------------------------- |
| `DNS`, `DNSOption`, `DNSSearch`, `ExposeHostPort`, `Annotation` | Repeatable  | 5.4.0–6.1.0                               |
| `IP`, `IP6`                                                     | Singleton   | 5.4.0–6.1.0                               |
| `NetworkAlias`                                                  | Repeatable  | 5.4.0–6.1.0                               |
| `AppArmor`                                                      | Singleton   | 5.8.0–6.1.0; unsupported through 5.7.1    |
| `NoNewPrivileges`, `SeccompProfile`, `SecurityLabel*`           | Singleton   | 5.4.0–6.1.0                               |
| `Mask`, `Unmask`                                                | Repeatable  | 5.4.0–6.1.0; earlier introduction unknown |
...
logical `--hostname app.example` argument across Podman 5.4.0 through 6.1.0. When a container joins
